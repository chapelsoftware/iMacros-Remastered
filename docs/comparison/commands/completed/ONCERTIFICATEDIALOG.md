# ONCERTIFICATEDIALOG Command Comparison
...
**New parser**: Listed in no-validation category (parser.ts:937) — the parser accepts it with any parameters or none.

## Parameters

| Parameter | Description | Old | New |
|-----------|-------------|-----|-----|
| BUTTON | Button to click on certificate dialog | Accepted but never used (throws immediately) | OK, CANCEL, YES, NO (defaults to OK if omitted; unrecognized values default to CANCEL) |

## Old Implementation (MacroPlayer.js:1129-1132)
...
1. **Regex match**: The pattern `.*` matches any input after the command name, so `ONCERTIFICATEDIALOG BUTTON=OK` or any other parameter combination is accepted by the parser.
2. **Throw UnsupportedCommand**: Immediately throws an `UnsupportedCommand` exception with the command name `"ONCERTIFICATEDIALOG"`. No parameter processing occurs.
...
- **Firefox limitation**: SSL certificate dialogs in Firefox are handled at the browser level, outside the reach of the extension's XPCOM-based API.

## New Implementation

### Handler (dialogs.ts:517-552 — `onCertificateDialogHandler`)
...
### Unsupported override (unsupported.ts:77-80)
...
In the extension context, the unsupported handler overrides the dialog handler (unsupported.ts:201), returning `UNSUPPORTED_COMMAND` error code (-915). The functional handler in `dialogs.ts` is available for non-extension contexts (e.g., native host) where certificate interception may be possible.
...
5. **Store in state**: Sets the `!CERTIFICATE_BUTTON` variable in macro state so the configuration persists during macro execution.
6. **Send to extension**: Sends a `CERTIFICATE_CONFIG` message through the dialog bridge with `{ button, active: true }`.
...
1. **Return error immediately**: Returns `{ success: false, errorCode: UNSUPPORTED_COMMAND (-915) }` with message `"ONCERTIFICATEDIALOG is not supported: Certificate dialog handling is not available in browser extensions"`.

### Message flow

```
onCertificateDialogHandler → sendDialogMessage → DialogBridge.sendMessage
                                                 ↓
                                          Extension background script
                                          (configures certificate dialog handling)
```

### Data types

```typescript
interface CertificateDialogConfig {
  button: DialogButton;    // 'OK' | 'CANCEL' | 'YES' | 'NO'
  remember?: boolean;      // Whether to remember the decision
...
| **State storage** | N/A | Sets `!CERTIFICATE_BUTTON` variable | **Enhancement**: State tracked for potential future use |
...
- **Variables modified**: `!CERTIFICATE_BUTTON` (set to button value in functional handler)
- **Extension context**: Returns `UNSUPPORTED_COMMAND` error — no side effects
- **Non-extension context**: Sends `CERTIFICATE_CONFIG` message through dialog bridge
- **No DOM side effects**
...
- `ONCERTIFICATEDIALOG BUTTON=OK` sends `CERTIFICATE_CONFIG` with `button=OK, active=true`
- `ONCERTIFICATEDIALOG BUTTON=CANCEL` sends config with `button=CANCEL`
...
- `ONCERTIFICATEDIALOG BUTTON=CANCEL` sends CANCEL config through bridge
- Unrecognized button (e.g., `BUTTON=FOOBAR`) defaults to CANCEL

### Unit tests (`tests/unit/unsupported-commands.test.ts`)
- `ONCERTIFICATEDIALOG BUTTON=OK` returns `UNSUPPORTED_COMMAND` error in extension context
- Error message contains `"ONCERTIFICATEDIALOG"`
...
- `ONCERTIFICATEDIALOG BUTTON=OK` parses with type `ONCERTIFICATEDIALOG`
