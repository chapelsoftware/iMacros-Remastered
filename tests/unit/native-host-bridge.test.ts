...
describe('BrowserBridge', () => {
  let bridge: ReturnType<typeof createBrowserBridge>;
  ...
  describe('Navigation', () => {
    test('navigate sends correct message', async () => {
      ...
      expect((sentMessage as { type: string }).type).toBe('browser_command');
      expect((sentMessage as { payload: { commandType: string } }).payload.commandType).toBe('navigate');
      expect((sentMessage as { payload: { url: string } }).payload.url).toBe('https://example.com');
    ...
    test('getCurrentUrl sends correct message', async () => {
      ...
      expect((sentMessage as { payload: { commandType: string } }).payload.commandType).toBe('getCurrentUrl');
    ...
    test('goBack sends correct message', async () => {
      ...
      expect((sentMessage as { payload: { commandType: string } }).payload.commandType).toBe('goBack');
    ...
    test('refresh sends correct message', async () => {
      ...
      expect((sentMessage as { payload: { commandType: string } }).payload.commandType).toBe('refresh');
  ...
  describe('Tab Management', () => {
    test('openTab sends correct message', async () => {
      ...
      expect((sentMessage as { payload: { commandType: string } }).payload.commandType).toBe('openTab');
      expect((sentMessage as { payload: { url: string } }).payload.url).toBe('https://google.com');
    ...
    test('switchTab sends correct message', async () => {
      ...
      expect((sentMessage as { payload: { commandType: string } }).payload.commandType).toBe('switchTab');
      // Tab index is converted from 1-based to 0-based
      expect((sentMessage as { payload: { tabIndex: number } }).payload.tabIndex).toBe(1);
    ...
    test('closeTab sends correct message', async () => {
      ...
      expect((sentMessage as { payload: { commandType: string } }).payload.commandType).toBe('closeTab');
  ...
  describe('Frame Management', () => {
    test('selectFrame sends correct message', async () => {
      ...
      expect((sentMessage as { payload: { commandType: string } }).payload.commandType).toBe('selectFrame');
      expect((sentMessage as { payload: { frameIndex: number } }).payload.frameIndex).toBe(1);
  ...
  describe('Interaction Commands', () => {
    test('executeTag sends correct message', async () => {
      ...
      expect((sentMessage as { payload: { commandType: string } }).payload.commandType).toBe('TAG_COMMAND');
      expect((sentMessage as { payload: { selector: { pos: number } } }).payload.selector.pos).toBe(1);
      expect((sentMessage as { payload: { selector: { type: string } } }).payload.selector.type).toBe('INPUT');
      expect((sentMessage as { payload: { action: { content: string } } }).payload.action.content).toBe('testuser');
    ...
    test('executeClick sends correct message', async () => {
      ...
      expect((sentMessage as { payload: { commandType: string } }).payload.commandType).toBe('CLICK_COMMAND');
      expect((sentMessage as { payload: { x: number } }).payload.x).toBe(100);
      expect((sentMessage as { payload: { y: number } }).payload.y).toBe(200);
    ...
    test('executeEvent sends correct message', async () => {
      ...
      expect((sentMessage as { payload: { commandType: string } }).payload.commandType).toBe('EVENT_COMMAND');
      expect((sentMessage as { payload: { eventType: string } }).payload.eventType).toBe('click');
      expect((sentMessage as { payload: { selector: { css: string } } }).payload.selector.css).toBe('.my-button');
...
describe('CommandHandlers', () => {
  let mockBridge: {
    ...
    selectFrame: ReturnType<typeof vi.fn>;
    selectFrameByName: ReturnType<typeof vi.fn>;
    executeTag: ReturnType<typeof vi.fn>;
    executeClick: ReturnType<typeof vi.fn>;
    executeEvent: ReturnType<typeof vi.fn>;
  };
  ...
  function createMockContext(params: Record<string, string>) {
    ...
    return {
      ...
      getRequiredParam: (key: string) => {
        ...
        if (!val) throw new Error(`Missing required parameter: ${key}`);
        return val;
      },
      expand: (text: string) => text,
      log: vi.fn(),
